...
class FieldLangTest extends BaseCase
{
    ...
    {
        $this->fieldLang->init($this->mainConfig)->createFieldLangFile($this->fieldsData);
        ...
        // specific lang
        $this->fieldLang->init($this->mainConfig)->createFieldLangFile($this->fieldsData, 'de-de');
    ...
    */
    ...
    {
        $this->fieldLang->init($this->mainConfig)->removeFieldLangFile();
        $filePath = createPath(base_path(), 'api', 'lang', 'field', 'en-us', 'UnitTest') . '.php';
        $this->assertFalse($this->fileSystem->exists($filePath));
        // specific lang
        $this->fieldLang->init($this->mainConfig)->removeFieldLangFile('de-de');
        $filePath = createPath(base_path(), 'api', 'lang', 'field', 'de-de', 'UnitTest') . '.php';
        $this->assertFalse($this->fileSystem->exists($filePath));
    }
}
